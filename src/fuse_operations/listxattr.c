/*
   Tagsistant (tagfs) -- fuse_operations/listxattr.c
 ...
 */
int tagsistant_listxattr(const char *path, char *list, size_t size)
{
    int res = 0, tagsistant_errno = 0;

	TAGSISTANT_START(OPS_IN "SETXATTR on %s", path);
	...
	// -- archive --
		...
		if (!g_regex_match_simple(TAGSISTANT_INODE_DELIMITER, qtree->object_path, 0, 0)) {
			res = llistxattr(qtree->object_path, list, size);
			tagsistant_errno = errno;
		} else if (qtree->full_archive_path) {
			res = llistxattr(qtree->full_archive_path, list, size);
			tagsistant_errno = errno;
		} else {
			TAGSISTANT_ABORT_OPERATION(ENOENT);
		}
...
TAGSISTANT_EXIT_OPERATION:
	...
	if ( res is -1 ) {
		TAGSISTANT_STOP_ERROR(OPS_OUT "LISTXATTR on %s {%s}: %d %d: %s", path, tagsistant_querytree_type(qtree), res, tagsistant_errno, strerror(tagsistant_errno));
		tagsistant_querytree_destroy(qtree, TAGSISTANT_ROLLBACK_TRANSACTION);
		return (-tagsistant_errno);
	} else {
		TAGSISTANT_STOP_OK(OPS_OUT "LISTXATTR on %s {%s}: OK", path, tagsistant_querytree_type(qtree));
