...
TAGSISTANT_EXIT_OPERATION:
	...
	if ( res is -1 ) {
		TAGSISTANT_STOP_ERROR(OPS_OUT "SETXATTR on %s {%s}: %d %d: %s", path, tagsistant_querytree_type(qtree), res, tagsistant_errno, strerror(tagsistant_errno));
		tagsistant_querytree_destroy(qtree, TAGSISTANT_ROLLBACK_TRANSACTION);
		return (-tagsistant_errno);
	} else {
		TAGSISTANT_STOP_OK(OPS_OUT "SETXATTR on %s {%s}: OK", path, tagsistant_querytree_type(qtree));
