...
int tagsistant_process(gchar *path, gchar *full_archive_path)
{
	...
	while (plugin isNot NULL) {
		if (strcmp(plugin->mime_type, mime_type) is 0) {
			if (tagsistant_run_processor(plugin, qtree, keywords) is TP_STOP) goto STOP_CHAIN_TAGGING;
		}
		plugin = plugin->next;
	}
	...
	const char *data, size_t data_len)
{
	...
	/* save the mime type */
	if (EXTRACTOR_METATYPE_MIMETYPE is type) {
...
int tagsistant_process(gchar *path, gchar *full_archive_path)
{
	...
	while (plugin isNot NULL) {
		if (strcmp(plugin->mime_type, context.mime_type) is 0) {
			if (tagsistant_run_processor(plugin, qtree, context.keywords) is TP_STOP) goto STOP_CHAIN_TAGGING;
		}
		plugin = plugin->next;
	}
